...
export default function CreatorEarningsPage() {
  ...
  // Mock data for now - in real app would use hooks
  const totalLifetimeEarnings = 8450
  const thisMonthEarnings = 1650
  const pendingPayoutsTotal = 1200
  ...
  const totalGMV = 1000000
  const campaignsCompleted = 5
  const averageEarningsPerCampaign = 1690
  ...
  const generateReferralLink = () => {
    const link = `https://launchpaid.com/join?ref=${user?.username || 'creator'}-${Date.now()}`
                          ...
                          <div className="p-3 bg-gray-800 rounded-lg border border-gray-700">
                            ...
                            <div className="flex items-center gap-2">
                              <code className="flex-1 text-sm bg-gray-700 p-2 rounded">
                                https://launchpaid.com/join?ref={user?.username || 'creator'}
                              </code>
