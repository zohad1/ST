...
async def apply_to_campaign(
    ...
    try:
        ...
        result = await service.create_application(application, creator_id=UUID(current_user["id"]))
...
async def get_my_applications(
    ...
    try:
        service = ApplicationService(db)
        applications = await service.get_creator_applications(creator_id=UUID(current_user["id"]))
...
async def get_campaign_applications(
    ...
    try:
        service = ApplicationService(db)
        applications = await service.get_campaign_applications(campaign_id, status_filter)
...
async def get_application_statistics(
    ...
    try:
        service = ApplicationService(db)
        stats = await service.get_application_statistics(campaign_id)
...
async def get_applications_with_filters(
    ...
    try:
        service = ApplicationService(db)
        result = await service.get_applications_with_filters(
            search=search,
            status=status,
