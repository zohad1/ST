...
async def submit_deliverable(
    ...
    return await service.submit_deliverable(deliverable, creator_id=UUID(current_user["id"]))
...
async def get_my_deliverables(
    ...
    service = DeliverableService(db)
    return await service.get_creator_deliverables(
...
async def get_campaign_deliverables_for_creator(
    ...
    service = DeliverableService(db)
    return await service.get_campaign_deliverables_for_creator(
        campaign_id=campaign_id,
        creator_id=UUID(current_user["id"])
    )
...
async def get_campaign_deliverables(
    ...
    service = DeliverableService(db)
    return await service.get_campaign_deliverables(campaign_id, status_filter)