# app/api/endpoints/dashboard.py - Async version
...
async def get_dashboard_analytics(
    ...
    try:
        ...
        current_campaigns = result.scalars().all()
        ...
        previous_campaigns = result.scalars().all()
        ...
        if current_campaigns:
            ...
            result = await db.execute(
                select(CampaignApplication.creator_id).where(
                    CampaignApplication.campaign_id.in_(campaign_ids),
                    CampaignApplication.status == "approved"
                ).distinct()
            )
            current_active_creators = len(result.scalars().all())
        ...
        if previous_campaigns:
            ...
            previous_active_creators = len(result.scalars().all())
        ...
        result = await db.execute(
            select(CampaignApplication, User).join(
        ...
        applications_with_creators = result.all()
...
async def get_dashboard_campaigns(
    ...
    try:
        ...
        result = await db.execute(
            query.order_by(desc(Campaign.created_at)).offset(offset).limit(limit)
        )
        campaigns = result.scalars().all()
...
async def get_creator_performance(
    ...
    try:
        ...
        query = select(CampaignApplication, User).join(
            User, CampaignApplication.creator_id == User.id
        ).where(
            CampaignApplication.status == "approved",
        ...
        if campaign_id:
            try:
                ...
                query = query.where(CampaignApplication.campaign_id == campaign_uuid)
            except ValueError:
        ...
        result = await db.execute(
            query.order_by(desc(CampaignApplication.previous_gmv)).limit(limit)
        )
        applications_with_creators = result.all()
